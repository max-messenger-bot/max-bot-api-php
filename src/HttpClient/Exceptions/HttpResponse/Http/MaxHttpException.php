...
/**
 * Базовый класс для HTTP-исключений API Max.
 */
abstract class MaxHttpException extends HttpException
{
    ...
    /**
     * @return Error The error details from the API response.
     */
    ...
    final public static function throwMax(HttpResponseInterface $response, Error $error): never
    {
        throw match ($response->getHttpCode()) {
            400 => new BadRequestException($response, $error),
            ...
            415 => new UnsupportedMediaTypeException($response, $error),
            429 => new TooManyRequestsException($response, $error),
            500 => new InternalHttpException($response, $error),
            503 => new ServiceUnavailableException($response, $error),
