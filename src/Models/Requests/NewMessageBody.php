...
final class NewMessageBody extends BaseRequestModel
{
    ...
    {
        if ($mid instanceof Message) {
            $mid = $mid->getBody()->getMid();
        } elseif ($mid instanceof MessageBody) {
            $mid = $mid->getMid();
        }

        $this->data['link'] = new NewMessageLink($mid, MessageLinkType::Forward);
    ...
    public function setReplyLink(string|Message|MessageBody $mid): self
    {
        ...
        $this->data['link'] = new NewMessageLink($mid, MessageLinkType::Reply);
