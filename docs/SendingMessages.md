...
```php
use MaxMessenger\Bot\Model\Request\CallbackButton;
use MaxMessenger\Bot\Model\Request\LinkButton;
use MaxMessenger\Bot\Model\Request\NewMessageBody;
...
В один диалог, групповой чат или канал можно отправлять не более двух сообщений в секунду. Такой же лимит действует
на редактирование и удаление сообщений, а также на ответы на нажатия кнопок (`answerOnCallback()`).

При превышении лимита сообщения следует ставить в очередь или делать задержку перед отправкой, иначе сервер ответит
ошибкой `TooManyRequestsException` (429).
